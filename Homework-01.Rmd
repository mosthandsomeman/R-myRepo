...
```{r}
#a
iowa.df<-data.frame(read.csv("data/iowa.csv", sep = ";"))
```
...
plot(dataframe.orings$Temperature, dataframe.orings$Total)
...
mtext("Y unit is [2**y - 2**(y-1)]", side = 2)
mtext("#eg:[1151, Gods],1151 is the lake's area,Gods is the lake's name", side = 1, line = 2);
```
Devise captions that explain the labeling on the points and on the y-axis. It will be necessary to explain how distances on the scale
...
```{r}
dotchart(area,labels=row.names(Manitoba.lakes), main="Lakes' area",xlab='area')
dotchart(log2(area),labels=row.names(Manitoba.lakes), main="Lakes' area",xlab='log2(area)')
```
